...
<script>
    ...
    $(document).ready(function() {
        <?= (Request::is('dosen*')) ? "$('#sidebarPages').collapse('show')" : ''; ?>
    ...
    function filterData() {
        ...
        var totPagesLoad = 5
        var dataUrl = "<?= url('dosen/get-all-data') ?>"
        var dataBody = {
    ...
    function openModal() {
        openSyncModal('/dosen/syncronize');
    }

    function openSyncModal(syncUrl) {
        Swal.fire({
            title: 'Sinkronisasi Data Dosen',
            text: 'Apakah anda yakin ingin melakukan sinkronisasi data dosen?',
            icon: 'warning',
